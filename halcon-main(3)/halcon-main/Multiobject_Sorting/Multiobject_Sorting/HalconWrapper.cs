...
namespace Multiobject_Sorting
{
    public class HalconWrapper
    {
        ...
        public HObject HSVColorSegmentation(HSVParameters parameters)
        {
            ...
            {
                // 转换为HSV颜色空间
                HOperatorSet.Decompose3(currentImage, out HObject red, out HObject green, out HObject blue);
                HOperatorSet.TransFromRgb(red, green, blue, out HObject hueImage, out HObject saturationImage, out HObject valueImage, "hsv");
        ...
        public Tuple<int, int> GetImageSize()
        {
            try
            {
                if (currentImage != null)
                {
                    ...
                    return Tuple.Create(width.I, height.I); // 返回传统 Tuple
                }
